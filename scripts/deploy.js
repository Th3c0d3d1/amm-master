...
async function main() {
  ...
  // Deploy Token 1
  let ssm = await Token.deploy('Saucy Sam', 'SSM', '1000000') // 1 million tokens
  await ssm.deployed()
  console.log(`SSM Token deployed to: ${ssm.address}\n`)
  ...
  const amm = await AMM.deploy(ssm.address, usd.address)
