...
async function main() {
  ...
  // Fetch SSM Token
  const ssm = await ethers.getContractAt('Token', config[chainId].ssm.address)
  console.log(`SSM Token fetched: ${ssm.address}\n`)
  ...
  // Send ssm tokens to investor 1
  transaction = await ssm.connect(deployer).transfer(investor1.address, tokens(10))
  ...
  // Send ssm tokens to investor 3
  transaction = await ssm.connect(deployer).transfer(investor3.address, tokens(10))
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 1 Swaps: SSM --> USD
  //

  console.log(`Investor 1 Swaps...\n`)

  // Investor approves all tokens
  transaction = await ssm.connect(investor1).approve(amm.address, tokens(10))
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 3 Swaps: SSM --> USD
  //

  console.log(`Investor 3 Swaps...\n`)

  // Investor approves all tokens
  transaction = await ssm.connect(investor3).approve(amm.address, tokens(10))
  await transaction.wait()
